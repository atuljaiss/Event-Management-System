    ...
    getRegAuthCheck: (req, res) => {
        ...
        var role = req.body.role;
            db.query('SELECT * FROM user_credencials WHERE Uusername = ? AND Upassword = ?', [username,password], function(error, results, fields, rows) {
                if (results.length === 0) {
                    db.query('INSERT INTO user_credencials (Uusername,Upassword,Urole) VALUES (?,?,?)', [username,password,role], function(error, results, fields, rows) {
                        if(role==1){
                            ...
                            db.query('SELECT Uid AS ID FROM user_credencials WHERE Uusername = ?', [username], function(error, results, fields, rows) {
                                db.query('INSERT INTO employee (EUid,Ename,Edept) VALUES (?,?,?)', [results[0].ID,name,dept], function(error, results, fields, rows) {
                                    db.query('SELECT Eid AS ID FROM employee WHERE Ename = ?', [name], function(error, results, fields, rows) {
                                        db.query('INSERT INTO events_organizer (EOname,EOdept,EOeid) VALUES (?,?,?)',[name+"_org",dept,results[0].ID],function(error,results,fields,rows){
                                            res.render('login.ejs',{
                                                title: 'Login',
                                                loginMSG:`${username} is Registered Succesfully`,
                                                regMSG:''
